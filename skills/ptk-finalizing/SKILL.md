...
Ship the completed work. By this point the stub frontier is empty — for every `.ptk-scaffold` sentinel, grepping its recorded pattern (first non-comment line) under its directory returns nothing — and no `it.todo`/`t.Skip` markers remain.
