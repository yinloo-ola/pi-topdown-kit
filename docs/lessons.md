...
- **When skills prescribe shell commands, keep guard allowlists in lock-step.** If a skill tells the model to run `X`, the blocking-phase guard must explicitly allow `X` (and tests should pin that pairing). Otherwise the workflow deadlocks on its own instructions.
- **Normalize command wrappers before policy matching.** Safety/repo-scope detection should parse through common wrappers (env assignments, `env`, `command`, absolute binary paths) before deciding allow/deny; matching only the raw first token creates easy bypasses and false negatives.